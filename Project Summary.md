...
scripts/                     Local analysis tooling (Node, service_role key) — NOT shipped to the browser
  ...
  liyp-check.mjs             Verifies LIYP the same way — group counts recounted independently, plus base/percentage reconciliation for all three dimensions
...
**Placeholder dates:** ASAP writes `1900-01-01` for a missing birthdate and `2050-01-01` for a missing class start date. Birthdates before 1905 fall to `No Response`; implausible start dates are nulled by `classStartDate()` in `src/utils/periodUtils.js` (before 2015, or more than five years ahead of today — the upper bound floats with the clock so it never needs revisiting). Both matter because age is computed *at the class start date* in Demographics, LIYP and the Board report.
...
The same figures the **Enrollment** page reports as a table, written out as prose for one selected quarter and compared against two others. It exists so a quarterly summary can be produced without transcribing a table by hand.

**Metrics** are the Enrollment page's own row definitions — total, fee-based, tuition-free, lessons, group classes, Mission, Richmond, and the four fee/free × lesson/class cells — so a number quoted here is the same number that page shows for the same quarter. Counts are enrollment rows; unique students are reported separately.

**Comparisons are fixed, not chosen.** One dropdown picks the quarter; both comparisons follow from it — the **same quarter one fiscal year earlier** (Fall 2026 against Fall 2025) and the **quarter immediately before it** (Fall 2026 against Summer 2026). They are deliberately not selectable: every summary then has the same shape, so two quarters' summaries can be read side by side without first checking what each was measured against. Both are named above the text, and `comparisonsFor()` is exported from the pure-logic block so the check script uses the report's own derivation rather than a copy.

A quarter with no earlier fiscal year on file gets no year-over-year paragraph rather than a misleading one.

**The prose is generated by rule, not by a language model.** Sentences are assembled from the numbers, and the report never says *why* anything moved — only what changed and by how much. Two rules keep it honest:

- A percentage is suppressed when the earlier quarter had fewer than 25 enrollments in that category. Three on a base of six is "+50%", which reads as a trend and isn't one; below the threshold the prose gives counts only.
- A movement under 10 enrollments *and* under 1.5% is described as holding steady rather than given a direction.

**Seasonal caveat:** because the sequential comparison is fixed, summer-to-fall pairings do occur — and summer terms are genuinely shorter, so that comparison largely measures the calendar. Rather than dropping it, the report measures each season's median quarter total across every quarter on file and appends a caveat sentence when two seasons differ in scale by more than 15% (summer runs ~51% the size of fall). The comparison is still shown, just labelled for what it is.
...
Grant report on four low-income youth cohorts for **one or more selected fiscal years** (FY pills, multi-select). For each group it shows **unique students** (counted once per FY; every enrollment already qualifies since only ENROLLED/PEND import) broken down by **household income**, **ethnicity** and **gender**. A de-duplicated **combined** total across all four groups is shown last. One CSV export covers all groups + combined.

**Categories and percentage bases** come from `demographicCategories.js`, so they are identical to the Demographics and Neighborhood Choir reports by construction (Hispanic + Latinx merged; trans labels merged to Transgender; income mapped by `INCOME_MAP`). Ethnicity and gender exclude only `No Response` from the base; **household income also excludes `Decline to State`**, so income percentages describe only students who named a bracket (see the Demographics **Percentage base** table above). Income + gender were added August 2026 at a colleague's request; before that the report showed ethnicity only, with its own hand-rolled `total − No Response` base — now replaced by the shared `pctBase`/`bucketPct` helpers so LIYP cannot drift from Demographics.

**Year-over-year comparison:** each group renders as **one table** (Enrollment-page `report-table` / `rt-*` / `delta-line` styling) with a column per selected FY and a **Δ column between consecutive years**. Rows are `Unique students` (bold), then — per dimension, in the order Household income · Ethnicity · Gender — an `rt-section-hdr` row followed by that dimension's category rows. All three dimensions share one table rather than getting three (the Neighborhood Choir layout) so the year columns stay aligned and there is a single Δ set; five tables instead of fifteen.

**The section header carries the percentage base per year** — the label reads e.g. *Household income — % of students who named a bracket* and each year column shows `of N`. This is load-bearing, not decoration: the bases differ per dimension *and* per group, so without it a `100.0% Low` cell reads as 100% of the group. In FY26 the income base is 110 of 220 Sliding-Scale students but only 9 of 34 Children's Chorus students. Section rows show no Δ share (a percentage of a percentage base means nothing); their Δ column stays blank on screen.

Category rows are the union across the selected years, so a category present in only one year still gets a row (0 elsewhere). Income rows use the fixed `INCOME_ORDER`; ethnicity and gender order by total count with `No Response` last. Each year is aggregated **independently**: a student enrolled in two selected years counts once in *each* column, so columns are not additive.

Δ cells show two lines: change in students, and change in share. Share change is a **relative %** on the `Unique students` row but **percentage points (pp)** on category rows — a category can grow in headcount while shrinking as a share. Categories excluded from a base have no share, so their second line is `—`. Caveat when reading pp shifts across years: the No Response count has been falling steadily, so part of any share movement reflects improving response rates, not a changing student mix.

**Reading the two new dimensions (documented in the About panel):**

- **Income on the Sliding-Scale group is close to circular** — qualifying for a sliding-scale or Merit discount is itself an income test, so that group reports **100.0% Low in every year on file**. YMP is the only group with real variation: 97.6% → 93.9% → 91.3% → 88.9% across FY23–FY26. Combined runs 99.6% → 98.9% → 98.2% → 96.9%. It is still the figure funders ask for, so it ships — but it is not a finding.
- **Gender is thinly answered in these youth cohorts**, much more thinly than ethnicity: `No Response` is 48–61% of the Sliding-Scale group and 50–55% of Combined across FY23–FY26 (FY26: 135 of 220 and 196 of 354). The gender percentages therefore speak for a minority of each group, which is why the base sits on the header row.

The CSV is tall — one row per (group, dimension, category), a Students/% pair per FY, then Δ Students and Δ "% / pp" pairs between consecutive years. Each dimension also emits a `Percentage base (…)` row so the spreadsheet carries the denominator the on-screen header shows.

**Data loading:** two-phase, unchanged apart from the join. Mount → paginated distinct `fiscal_year`. On FY selection → paginated (1000/batch) `enrollments` filtered by `fiscal_year`, joined to `events(course_name, class_start_date)` and `students(birthdate, ethnicity, gender, household_income)` — `gender` and `household_income` added for the two new dimensions. Group membership, dedup and bucketing are all client-side.

**Verification:** `node scripts/liyp-check.mjs` extracts the report file's own pure-logic block (between the `pure logic` markers) and runs it over live data — no reimplementation; it reuses the report's own import line for the shared categories, so a name added there can't leave the check behind. It recounts unique students per group straight off the raw rows (**FY23** sliding 353 / ymp 90 / chorus 38 / teen 20 / combined 479; **FY24** 351 / 97 / 35 / 18 / 479; **FY25** 299 / 89 / 40 / 24 / 429; **FY26** 220 / 97 / 34 / 19 / 354; **FY27** 99 / 0 / 0 / 0 / 99 — matching the counts the report showed before income and gender were added, which is the anchor proving the new dimensions changed no membership), reconciles `base + excluded = all students` and confirms percentages sum to 100.0000% of each base across **75 unit × dimension × year combinations**, asserts the income-only `Decline to State` exclusion, and checks the rendered table: every dimension's category rows sum to the unit's unique students, and the rows carrying a percentage sum to the base printed on its section header.

**Tuition assistance:** originally shown per group (dollars from `total_discount` for Sliding-Scale + YMP; fully-subsidized headcount for the free programs), but **removed at the user's request** — the tuition-free programs record `$0` discounts inconsistently in ASAP (a billing-practice artifact, not a real change in aid), so the dollar/subsidized figures confused more than helped. The report now shows only unique students + the three demographic breakdowns.
...
`fetchByIds(supabase, table, { column, values, select })` covers the other half: a PostgREST `.in()` filter travels **in the URL**, so a long list is simply a request too long to send. Three quarters of enrollments reference ~3,200 events — a 25,000-character URL and a flat `400` from the server, which surfaces as a generic network failure rather than "your list is too long". It chunks the ids 500 at a time and issues the chunks in parallel. Reach for it whenever a join is driven by a key list rather than a filter, and assert afterwards that nothing failed to join: a missed join nulls the joined columns silently, and a report counting on them quietly undercounts.
...
Readiness is judged by the app's own convention rather than one string: it waits until no `.coming-soon` element's text ends in an ellipsis. Matching the literal word "Loading" was not enough — a report that loads in two phases shows a second, differently worded placeholder ("Reading those quarters…"), and the script walked straight past it and screenshotted the spinner while reporting the page rendered cleanly. A genuine empty state ("Pick a quarter to summarise.") has no ellipsis, so it doesn't block.
...
- **NEEDS ATTENTION** — a rule actually failed: an unmatched discount code, an income bracket missing from `INCOME_MAP`, a YMP-looking course missing from LIYP's exact-match set, an orphaned or duplicated row, or `is_tuition_free` disagreeing with `(amount - discount) <= 15`.
- **note** — a judgement call for a human: discount codes and courses appearing for the first time, ethnicity/gender values never seen before (each becomes its own row unless aliased), blank departments, sections carrying ASAP's placeholder start date (harmless to the numbers since `classStartDate()` nulls them, but worth correcting in ASAP — it leaves those students with an unknown age), and whether the quarter looks complete.

The audit imports `classStartDate()` from `periodUtils.js` rather than restating the threshold, so its idea of a plausible date can't drift from the reports'.
...
| Placeholder class start dates | Fixed (September 2026) | ASAP writes `2050-01-01` as a section's start date when the real one is missing — the mirror of the `1900-01-01` birthdate. Demographics, LIYP and the Board report all compute age *at the class start date*, so a placeholder aged a student upward (a 40-year-old read as 65) and existing age guards missed it because the inflated age still looked plausible. `classStartDate()` in `periodUtils.js` now nulls an implausible date at every read point, and the reports already treat a null start as no date on record. Two sections affected (FY23 "Accompanist", FY25 "Fife"); both students had other same-quarter enrollments with real dates, so no published figure was wrong — it was luck, not design. Surfaced by `scripts/quarter-audit.mjs` |
