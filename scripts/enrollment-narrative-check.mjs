...
const enrollments = await fetchAll('enrollments', 'event_enrollment_id,event_id,customer_id,time_period,is_tuition_free')
...
for (const q of ordered) {
  ...
  const yoy   = unit(yoyP)
  const seq   = unit(seqP)
  const nar   = R.buildNarrative({ focus, yoy, seq, medians })
  scanned++
...
console.log('\n── Suppression rules ──')
{
  const mk = (enr, stu = enr) => ({ enr, stu })
  const stats = keys => Object.fromEntries(keys.map(([k, v]) => [k, mk(v)]))
  const base = ['total','fee','free','lessons','group','mission','richmond','feeLessons','feeGroup','freeLessons','freeGroup']
  const flat = k => stats(base.map(b => [b, k]))

  // A percentage on a tiny base must not appear.
  const small = R.buildNarrative({
    focus: { period: 'Fall Quarter 2026', rows: [], stats: { ...flat(10), free: mk(9), fee: mk(1) } },
    yoy:   { period: 'Fall Quarter 2025', rows: [], stats: { ...flat(10), free: mk(6), fee: mk(4) } },
    seq: null, medians: {},
  })
  check(!/50\.0%/.test(small.plainText), 'percentage quoted on a base below the minimum')

  // An unchanged quarter reads as unchanged, not as a movement.
  const same = R.buildNarrative({
    focus: { period: 'Fall Quarter 2026', rows: [], stats: flat(1000) },
    yoy:   { period: 'Fall Quarter 2025', rows: [], stats: flat(1000) },
    seq: null, medians: {},
  })
  check(/unchanged/.test(same.plainText), 'a quarter identical to the last does not read as unchanged')
  check(!/grew|fell/.test(same.plainText), 'an unchanged quarter is described as moving')
}
